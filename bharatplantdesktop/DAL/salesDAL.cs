...
namespace bharatplantdesktop.DAL
{
    internal class salesDAL
    {
        ...
        public DataTable SingleSelect(salesBLL salesbll)
        {

            SqlConnection conn = new SqlConnection(myconnstrng);
            DataTable SaleSingleSelectDt = new DataTable();
            try
            {
                String sql = "SELECT * from tbl_sales where serial=@serial";
                SqlCommand cmd = new SqlCommand(sql, conn);
                cmd.Parameters.AddWithValue("@serial", salesbll.serial);
                SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                conn.Open();
                adapter.Fill(SaleSingleSelectDt);
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message);
        ...
        {
            Dictionary<int, string> salesData = new Dictionary<int, string>();

            SqlConnection conn = new SqlConnection(myconnstrng);
            try
            {
                string sql = "SELECT serial, narration FROM tbl_sales";
                SqlCommand cmd = new SqlCommand(sql, conn);
                conn.Open();
                ...
                while (reader.Read())
                {
                    int saleSerial = Convert.ToInt32(reader["serial"]);
                    string saleNarration = reader["narration"].ToString();
                    salesData.Add(saleSerial, saleNarration);
                }
                reader.Close();
            }
            catch (Exception ex)
        ...
        public bool Insert(salesBLL salesbll, out int salesId)
        {
            ...
            try
            {
                string sql = "INSERT INTO tbl_sales (voucher, date, party_id, total_amount,narration, added_by) VALUES (@voucher, @date, @party_id, @total_amount,@narration, @added_by); SELECT SCOPE_IDENTITY()";
                SqlCommand cmd = new SqlCommand(sql, conn);

                cmd.Parameters.AddWithValue("@voucher", salesbll.voucher);
                cmd.Parameters.AddWithValue("@date", salesbll.date);
                cmd.Parameters.AddWithValue("@party_id", salesbll.party_id);
                cmd.Parameters.AddWithValue("@total_amount", salesbll.total_amount);
                cmd.Parameters.AddWithValue("@narration", salesbll.narration);
                cmd.Parameters.AddWithValue("@added_by", salesbll.added_by);
        ...
        public bool Update(salesBLL salesbll)
        {
            bool isSuccess = false;
            SqlConnection conn = new SqlConnection(myconnstrng);
            ...
            try
            {
                string sql = "UPDATE tbl_sales SET voucher = @voucher, date = @date, party_id = @party_id, total_amount = @total_amount, narration = @narration, added_by = @added_by WHERE serial = @serial";
                SqlCommand cmd = new SqlCommand(sql, conn);
        ...
        public bool Delete(salesBLL salesbll)
        {
            bool isSuccess = false;
            SqlConnection conn = new SqlConnection(myconnstrng);
            ...
            try
            {
                string sql = "DELETE FROM tbl_sales WHERE serial=@serial";
        ...
        public int GetLatestSerial()
        {
            ...
            try
            {
                string sql = "SELECT IDENT_CURRENT('tbl_sales') as latestSerial";
                SqlCommand cmd = new SqlCommand(sql, conn);
                conn.Open();
                object result = cmd.ExecuteScalar();
