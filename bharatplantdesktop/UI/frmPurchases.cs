...
namespace bharatplantdesktop.UI
{
    public partial class frmPurchases : Form
    {
        ...
        private Dictionary<int, string> allpurchasesData;
        ...
        purchasesDAL purchasesdal = new purchasesDAL();
        purchasesDetailsDAL purchasesdetailsdal = new purchasesDetailsDAL();
        ...
        private void frmPurchases_Load(object sender, EventArgs e)
        {
            ...
            latestSerial = purchasesdal.GetLatestSerial() + 1;
        ...
        private void RemoveSelectedRow()
        {
            ...
            {
                DataRowView rowView = dgvPurchases.SelectedRows[0].DataBoundItem as DataRowView;
                if (rowView != null)
                {
                    ...
                    dataView.Delete(dgvPurchases.SelectedRows[0].Index);
                }
            }
        }
        ...
        private void loadReportDataTable()
        {
            ...
            dt = purchasesdal.Select(filterPeriodFromforDatabase, filterPeriodToforDatabase, selectedFilterPartyId, selectedFilterUserId);
            ...
            ThemeColors.SetColumnWidthsInPercentage(dgvPurchases, 9, 9, 32, 8, 22, 12, 8);
            PopulateSortColumns();
        }

        private void dgvPurchases_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (dgvPurchases.Columns[e.ColumnIndex].Name == "Date")
            {
        ...
        private void DisplayTotalCount() {
            ...
            txtPurchasesCount.Text = "Total Count:  " + bindingSource.Count.ToString();
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            switch (keyData)
            {
                case Keys.Escape:
                    ...
                    {
                        tabControlPurchases.SelectedIndex = 1;
                        dgvPurchases.Focus();
                        clear(true);
                ...
                case Keys.Enter:
                    ...
                    if (activeControl != null)
                    {
                        switch (activeControl)
                        {
                            ...
                            default:
                                if (activeControl == dgvPurchases && dgvPurchases.CurrentRow != null)
                                {
                                    DataGridViewRow selectedRow = dgvPurchases.CurrentRow;
                                    HanddgvPurchasesleRowSelection(selectedRow);
                                }
        ...
        private void SetupAutoComplete()
        {
            ...
            allpurchasesData = purchasesdal.SelectAllforAutoSuggestion();
            ...
            autoCompleteDataforNarration.AddRange(allpurchasesData.Values.ToArray());
        ...
        {
            foreach (TabPage tabPage in tabControlPurchases.TabPages)
            {
                if (tabPage != tabControlPurchases.SelectedTab)
                {
            ...
            foreach (Control control in tabControlPurchases.SelectedTab.Controls)
            {
        ...
        private void tabControlPurchases_SelectedIndexChanged(object sender, EventArgs e)
        {
            HidingUnwantedTabPages();
            if (tabControlPurchases.SelectedIndex == 0)
            {
                txtVoucher.Focus();
            }
            else if (tabControlPurchases.SelectedIndex == 1)
            {
                txtFilterSearch.Focus();
            }
            isFirstPage = tabControlPurchases.SelectedIndex == 0;
            isSecondPage = tabControlPurchases.SelectedIndex == 1;
        }
        private void tabControlPurchases_KeyDown(object sender, KeyEventArgs e)
        {
            if (isSecondPage && !dgvPurchases.ContainsFocus && e.KeyCode == Keys.Down || e.KeyCode == Keys.Up)
            {
                dgvPurchases.Focus();
            }
        ...
        {
            if (e.RowIndex >= 0 && e.RowIndex < dgvPurchases.Rows.Count)
            {
                HanddgvPurchasesleRowSelection(dgvPurchases.Rows[e.RowIndex]);
            }
        }
        private void HanddgvPurchasesleRowSelection(DataGridViewRow selectedRow)
        {
            if (int.TryParse(selectedRow.Cells["Serial"].Value.ToString(), out int serial))
            {
                SingleSelectWithNextAndPrev(serial, 0);
                tabControlPurchases.SelectedIndex = 0;
        ...
        private void UpdateUIFromSaleData(DataTable saleData, int newSerial)
        {
            ...
            if (condition)
            {
                ...
                #region
                purchasesDetailsBLL purchasesdetailsbll = new purchasesDetailsBLL();
                purchasesdetailsbll.serial = newSerial;
                DataTable saleDetailData = purchasesdetailsdal.SingleSelect(purchasesdetailsbll);
        ...
        private void SingleSelectWithNextAndPrev(int serial, int serialIncrement)
        {
            ...
            purchasesbll.serial = newSerial;
            txtSerial.Text = newSerial.ToString();

            DataTable saleData = purchasesdal.SingleSelect(purchasesbll);
        ...
        {
            DataTable dt = purchasesdal.Select(filterPeriodFromforDatabase, filterPeriodToforDatabase, selectedFilterPartyId, selectedFilterUserId);
        ...
        private void PreparingpurchasesDT()
        {
        ...
        {
            if (tabControlPurchases.SelectedIndex != 1) {
                return;
            }
            tabControlPurchases.SelectedIndex = 0;
            if (dgvPurchases.SelectedRows.Count > 0)
            {
                ...
                txtCustomeDate.Text = dgvPurchases.SelectedRows[0].Cells[1].Value.ToString();
                ValidateDateTextBox(txtCustomeDate, cancelEventArgs);

                txtPartyName.Text = dgvPurchases.SelectedRows[0].Cells[2].Value.ToString();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            purchasesbll.voucher = txtVoucher.Text;
            purchasesbll.date = dateValueforDatabase;
            purchasesbll.party_id = selectedPartyId;
            purchasesbll.total_amount = Math.Round(totalAmount, 2);
            purchasesbll.narration = txtNarration.Text;
            purchasesbll.added_by = loggedInUserId;
            purchasesbll.purchases_details = purchasesDT;

            bool success = false;

            using (TransactionScope scope = new TransactionScope())
            {
                int purchasesSerial = -1;
                bool x = purchasesdal.Insert(purchasesbll, out purchasesSerial);

                foreach (DataGridViewRow row in dgvAddedItems.Rows)
                {
                    purchasesDetailsBLL purchasesdetailsbll = new purchasesDetailsBLL();

                    purchasesdetailsbll.item_id = int.Parse(row.Cells[0].Value?.ToString());
                    purchasesdetailsbll.quantity = decimal.Parse(row.Cells[2].Value?.ToString());
                    purchasesdetailsbll.rate = decimal.Parse(row.Cells[3].Value?.ToString());
                    purchasesdetailsbll.amount = Math.Round(decimal.Parse(row.Cells[5].Value?.ToString()), 2);
                    purchasesdetailsbll.serial = purchasesSerial;
                    purchasesdetailsbll.party_id = selectedPartyId;
                    purchasesdetailsbll.added_date = DateTime.Now;
                    purchasesdetailsbll.added_by = loggedInUserId;

                    bool y = purchasesdetailsdal.Insert(purchasesdetailsbll);
                ...
                if (success == true)
                {
                    ...
                    latestSerial = purchasesSerial + 1;
                    txtSerial.Text = (purchasesSerial + 1).ToString();
                    txtVoucher.Clear();
                    txtVoucher.Focus();
                    txtPartyName.Clear();
                    dgvAddedItems.Rows.Clear();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            purchasesbll.narration = txtNarration.Text;
            purchasesbll.added_by = loggedInUserId;
            purchasesbll.serial = serial;

            bool success = false;

            using (TransactionScope scope = new TransactionScope())
            {
                bool x = purchasesdal.Update(purchasesbll);

                purchasesdetailsbll.serial = serial;
                bool y = purchasesdetailsdal.Delete(purchasesdetailsbll);
                ...
                foreach (DataGridViewRow row in dgvAddedItems.Rows)
                {
                    ...
                    bool z = purchasesdetailsdal.Insert(purchasesdetailsbll);
        ...
        private void DeleteFunctionality(int serial)
        {
            ...
            using (TransactionScope scope = new TransactionScope())
            {
                bool x = purchasesdal.Delete(purchasesbll);

                purchasesdetailsbll.serial = purchasesbll.serial; // Use the same serial for deletion in both tables
        ...
        private void DeleteFunctionalityUsingShortcut()
        {
            if (dgvPurchases.SelectedRows.Count > 0)
            {
                ...
                bool isValidSerial = int.TryParse(dgvPurchases.SelectedRows[0].Cells[serialColumnIndex].Value.ToString(), out int serial);
                if (!isValidSerial)
                {
                    MessageBox.Show("Serial is not valid");
                    return;
        ...
        private void txtNarration_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                string[] suggestions = allpurchasesData.Values
        ...
        #region
        private void printDgvPurchases_PrintPage(object sender, PrintPageEventArgs e)
        {
            PrintDataGridView(dgvPurchases, e.Graphics, new Rectangle(100, 100, 500, 300));
        }
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            GenerateAndDownloadPDF(dgvPurchases, columnsToShow, columnWidths, tableLeftMargin, tableRightMargin);
        }
        ...
        private DataTable GetSaleDetails(DataGridView dataGridView, int rowIndex)
        {
            ...
            purchasesDetailsBLL purchasesDetails = new purchasesDetailsBLL();
            purchasesDetails.serial = serial;
            DataTable saleDetailsTable = purchasesdetailsdal.SingleSelect(purchasesDetails);
