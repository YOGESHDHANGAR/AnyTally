...
namespace bharatplantdesktop.UI
{
    public partial class frmPayments : Form
    {
        ...
        private Dictionary<int, string> allPaymentsData;
        ...
        paymentsDetailsDAL paymentsdetailsdal = new paymentsDetailsDAL();
        ...
        private void frmPayments_Load(object sender, EventArgs e)
        {
            ...
            latestSerial = paymentsdal.GetLatestSerial() + 1;
        ...
        private void RemoveSelectedRow()
        {
            ...
            {
                DataRowView rowView = dgvPayments.SelectedRows[0].DataBoundItem as DataRowView;
                if (rowView != null)
                {
                    ...
                    dataView.Delete(dgvPayments.SelectedRows[0].Index);
                }
            }
        }
        ...
        private void loadReportDataTable()
        {
            ...
            dt = paymentsdal.Select(filterPeriodFromforDatabase, filterPeriodToforDatabase, selectedFilterPartyId, selectedFilterUserId);
            ...
            ThemeColors.SetColumnWidthsInPercentage(dgvPayments, 9, 9, 32, 8, 22, 12, 8);
            PopulateSortColumns();
        }

        private void dgvPayments_CellFormatting(object sender, DataGridViewCellFormattingEventArgs e)
        {
            if (dgvPayments.Columns[e.ColumnIndex].Name == "Date")
            {
        ...
        {
            txtPaymentsCount.Text = "Total Count:  " + bindingSource.Count.ToString();
        }
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            switch (keyData)
            {
                case Keys.Escape:
                    ...
                    {
                        tabControlPayments.SelectedIndex = 1;
                        dgvPayments.Focus();
                        clear(true);
                ...
                case Keys.Enter:
                    ...
                    if (activeControl != null)
                    {
                        switch (activeControl)
                        {
                            ...
                            default:
                                if (activeControl == dgvPayments && dgvPayments.CurrentRow != null)
                                {
                                    DataGridViewRow selectedRow = dgvPayments.CurrentRow;
                                    HandledgvPaymentsleRowSelection(selectedRow);
                                }
        ...
        private void SetupAutoComplete()
        {
            ...
            allPaymentsData = paymentsdal.SelectAllforAutoSuggestion();
            ...
            autoCompleteDataforNarration.AddRange(allPaymentsData.Values.ToArray());
        ...
        {
            foreach (TabPage tabPage in tabControlPayments.TabPages)
            {
                if (tabPage != tabControlPayments.SelectedTab)
                {
            ...
            foreach (Control control in tabControlPayments.SelectedTab.Controls)
            {
        ...
        private void tabControlPayments_SelectedIndexChanged(object sender, EventArgs e)
        {
            HidingUnwantedTabPages();
            if (tabControlPayments.SelectedIndex == 0)
            {
                txtVoucher.Focus();
            }
            else if (tabControlPayments.SelectedIndex == 1)
            {
                txtFilterSearch.Focus();
            }
            isFirstPage = tabControlPayments.SelectedIndex == 0;
            isSecondPage = tabControlPayments.SelectedIndex == 1;
        }
        private void tabControlPayments_KeyDown(object sender, KeyEventArgs e)
        {
            if (isSecondPage && !dgvPayments.ContainsFocus && e.KeyCode == Keys.Down || e.KeyCode == Keys.Up)
            {
                dgvPayments.Focus();
            }
        ...
        private void dgvPayments_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                HandledgvPaymentsleRowSelection(dgvPayments.Rows[e.RowIndex]);
            }
        }
        private void HandledgvPaymentsleRowSelection(DataGridViewRow selectedRow)
        {
            if (int.TryParse(selectedRow.Cells["Serial"].Value.ToString(), out int serial))
            {
                SingleSelectWithNextAndPrev(serial, 0);
                tabControlPayments.SelectedIndex = 0;
        ...
        private void UpdateUIFromPaymentData(DataTable paymentData, int newSerial)
        {
            bool condition = paymentData != null && paymentData.Rows.Count > 0;
            btnUpdate.Enabled = condition;
            btnDelete.Enabled = condition;
            if (condition)
            {
                txtVoucher.Text = paymentData.Rows[0]["voucher"].ToString();

                #region
                string dateStringFromDatabase = paymentData.Rows[0]["date"].ToString();
                ...
                string dateStr = (paymentData.Rows[0]["date"].ToString());
                DateTime date;
                ...
                int partyId;
                if (int.TryParse(paymentData.Rows[0]["party_id"].ToString(), out partyId))
                {
                ...
                #region
                paymentsDetailsBLL paymentsdetailsbll = new paymentsDetailsBLL();
                paymentsdetailsbll.serial = newSerial;
                DataTable saleDetailData = paymentsdetailsdal.SingleSelect(paymentsdetailsbll);
                ...
                txtTotalAmount.Text = paymentData.Rows[0]["total_amount"].ToString();
                txtNarration.Text = paymentData.Rows[0]["narration"].ToString();
                listBoxNarration.Visible = false;

                #region
                usersBLL usersbll = new usersBLL();
                int userId;
                if (int.TryParse(paymentData.Rows[0]["added_by"].ToString(), out userId))
                {
                    usersbll.id = userId;
        ...
        private void SingleSelectWithNextAndPrev(int serial, int serialIncrement)
        {
            ...
            paymentsbll.serial = newSerial;
            txtSerial.Text = newSerial.ToString();

            DataTable paymentsData = paymentsdal.SingleSelect(paymentsbll);
            UpdateUIFromPaymentData(paymentsData, newSerial);
        }
        ...
        {
            DataTable dt = paymentsdal.Select(filterPeriodFromforDatabase, filterPeriodToforDatabase, selectedFilterPartyId, selectedFilterUserId);
        ...
        private void PreparingpaymentsDT()
        {
        ...
        {
            if (tabControlPayments.SelectedIndex != 1)
            {
                return;
            }
            tabControlPayments.SelectedIndex = 0;
            if (dgvPayments.SelectedRows.Count > 0)
            {
                ...
                txtCustomeDate.Text = dgvPayments.SelectedRows[0].Cells[1].Value.ToString();
                ValidateDateTextBox(txtCustomeDate, cancelEventArgs);

                txtPartyName.Text = dgvPayments.SelectedRows[0].Cells[2].Value.ToString();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            paymentsbll.voucher = txtVoucher.Text;
            paymentsbll.date = dateValueforDatabase;
            paymentsbll.party_id = selectedPartyId;
            paymentsbll.total_amount = Math.Round(totalAmount, 2);
            paymentsbll.narration = txtNarration.Text;
            paymentsbll.added_by = loggedInUserId;
            paymentsbll.payments_details = paymentsDT;

            bool success = false;

            using (TransactionScope scope = new TransactionScope())
            {
                int paymentsSerial = -1;
                bool x = paymentsdal.Insert(paymentsbll, out paymentsSerial);

                foreach (DataGridViewRow row in dgvAddedItems.Rows)
                {
                    paymentsDetailsBLL paymentsdetailsbll = new paymentsDetailsBLL();

                    paymentsdetailsbll.transaction_mode_id = int.Parse(row.Cells[0].Value?.ToString());
                    paymentsdetailsbll.amount = Math.Round(decimal.Parse(row.Cells[2].Value?.ToString()), 2);
                    paymentsdetailsbll.serial = paymentsSerial;
                    paymentsdetailsbll.party_id = selectedPartyId;
                    paymentsdetailsbll.added_date = DateTime.Now;
                    paymentsdetailsbll.added_by = loggedInUserId;

                    bool y = paymentsdetailsdal.Insert(paymentsdetailsbll);
                ...
                if (success == true)
                {
                    ...
                    latestSerial = paymentsSerial + 1;
                    txtSerial.Text = (paymentsSerial + 1).ToString();
                    txtVoucher.Clear();
                    txtVoucher.Focus();
                    txtPartyName.Clear();
                    dgvAddedItems.Rows.Clear();
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            paymentsbll.narration = txtNarration.Text;
            paymentsbll.added_by = loggedInUserId;
            paymentsbll.serial = serial;

            bool success = false;

            using (TransactionScope scope = new TransactionScope())
            {
                bool x = paymentsdal.Update(paymentsbll);

                paymentsdetailsbll.serial = serial;
                bool y = paymentsdetailsdal.Delete(paymentsdetailsbll);
                ...
                foreach (DataGridViewRow row in dgvAddedItems.Rows)
                {
                    ...
                    bool z = paymentsdetailsdal.Insert(paymentsdetailsbll);
        ...
        private void DeleteFunctionality(int serial)
        {
            ...
            using (TransactionScope scope = new TransactionScope())
            {
                bool x = paymentsdal.Delete(paymentsbll);

                paymentsdetailsbll.serial = paymentsbll.serial; // Use the same serial for deletion in both tables
        ...
        private void DeleteFunctionalityUsingShortcut()
        {
            if (dgvPayments.SelectedRows.Count > 0)
            {
                ...
                bool isValidSerial = int.TryParse(dgvPayments.SelectedRows[0].Cells[serialColumnIndex].Value.ToString(), out int serial);
                if (!isValidSerial)
                {
                    MessageBox.Show("Serial is not valid");
                    return;
        ...
        private void txtNarration_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                string[] suggestions = allPaymentsData.Values
        ...
        #region
        private void printDgvPayments_PrintPage(object sender, PrintPageEventArgs e)
        {
            PrintDataGridView(dgvPayments, e.Graphics, new Rectangle(100, 100, 500, 300));
        }
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            GenerateAndDownloadPDF(dgvPayments, columnsToShow, columnWidths, tableLeftMargin, tableRightMargin);
        }
        ...
        private DataTable GetSaleDetails(DataGridView dataGridView, int rowIndex)
        {
            ...
            paymentsDetailsBLL paymentsDetails = new paymentsDetailsBLL();
            paymentsDetails.serial = serial;
            DataTable saleDetailsTable = paymentsdetailsdal.SingleSelect(paymentsDetails);
